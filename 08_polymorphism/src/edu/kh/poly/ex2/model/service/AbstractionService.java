...
import edu.kh.poly.ex2.model.vo.YJYCalculator;
...
//		Animal a = new Animal() { // Cannot instantiate the type Animal
		...
		p1.setType("포유류"); // 자식 객체 내부에 부모 부분이 있을을 확인
		
		// 추상 클래스는부모 타입의 참조 변수로 사용할 수 있다!
		// -> 다형성의 없 캐스팅
		Animal a1 = new Fish();
		...
		a1.breath();
		a2.breath();
		// 실행 전에 Animal의 breath()와 연결 된 것으로 보임(정적 바인딩)
		...
		// 다들 강제 오버라이딩한 breath()라는 공통적이 메서드를
...
//		KH kh1 = new KH(); // Cannot instantiate the type KH
	...
	public void ex3() {
		...
		Calculator cal = new YJYCalculator();
