...
//		c2.setWeight(1.5); // 에러
		...
		// 정적 바인딩 : 프로그램 실행 전 메서드 허출부 - 메서드 코드 연결
		// 			  부모 타입으로 메서드 호출 - 부모 클래스 메서드 연결
		
		// 동적 바인딩 : 프로그램 실행 중 
		// 		 	  참조하는 객체의 실제 타입의 오버라이딩 된 메서드 연결
		// 
	...
	public void ex2() {
		...
		arr[1] = new Truck(8, 3, "경유", 5.0); // Truck 객체
		// Car 참조 변수
...
//	}
	...
	public void ex3() {
		...
		// 	 부모 참조 변수를 자식 타입으로 바꿔(강제 형변화, 얕은 복사)
		//   자식 객ㅊ를 온전하게 참조할 수 있게 만듦.
...
//		( (Truck) c3 ).loading();
		...
		// 코드 상에 오류는 발생 X
		// ClassCastExeption : 형변환 예외
		//    -> 다운 캐스팅이 잘못된 경우 발생
		
		// * 해결 방법 : instacneof 연산자로 다운캐스팅 할 타입이 맞는지 검사
