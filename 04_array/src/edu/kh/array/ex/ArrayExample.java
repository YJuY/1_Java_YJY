...
public class ArrayExample {
	...
	public void ex4() {
		...
		// Arrays. toString(arr) : 배열데 저장된 모든 값을 한 줄로 간단히 출력
	...
	public void ex7() {
		...
		// 이원 수를 입력 받아 그 크기만큼의 정수 배열을 선언 및 할당
		// 인원 수 만큼 점수를 입력 받아
		// 합계, 평균, 최고점, 최저점을 출력
		...
		int max = scoreArr[0];
		int min = scoreArr[0];
		// 배열의 첫 번재 인덱스 값을 최고/ 최저의 비교 기준으로 삼음
	...
	public void ex8() {
		...
		for(int i = 0; i<arr.length; i++) {
			...
			if( arr[i]== search) { // arr[i] 값이 입력 받은 값과 같다면
				System.out.print(i+"번째 인덱스에 존재합니다.");
