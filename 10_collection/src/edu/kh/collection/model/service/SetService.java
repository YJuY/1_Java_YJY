...
public class SetService {
	...
	public void ex1() {
		...
		// -> 컬렉션에 저장된 객체를 임의의 순서로 하났기 꺼내는 역할
		
		
		Iterator<String> it = set.iterator();
		
		// set.iterator();
		// Set에 저장된 객체를 이용해서 Itorator 객체를 만들어 반환
	...
	public void ex2() { 
		 ...
		 *     -> Studnet 오버라이딩
		 *     
		 * Hash라는 단어가 붙은 컬렉션이 중복은 판단하는 방법
		 *  -> Object.hashCode() 오버라이딩
		 *  
		 * Hash 함수 : 입력된 단어를 지정된 길이의 무작위 문장려로 변환하는 함수
		 * 
		 * hashCode() : 객체에 저장된 필드를 이용해서 만들어진 정수를 반환
		 * 
		 * 
		 * HashSet -> equals(), hashCode() 둥다 오버라이딩 되어 있어야 함
		 * 
		...
		set.add(new Student("김길순", 17, "서울시 중로구", 'F', 100) );
		set.add(new Student("홍길동", 15, "서울시 중구", 'M', 50) );
		
		
		
		Student s1 = new Student("김길순", 17, "서울시 중로구", 'F', 100);
		Student s2 = new Student("김길순", 17, "서울시 중로구", 'F', 100);
	...
	public void ex3() {
		...
		// 문자열이짐나 내용이 모두 숫자인 문자열이 존재
		// -> Wrapper 클래스의 paresXXX() 를 이용하면 숫자로 변경 가능
				
		System.out.println(num + 100); // 223
		System.out.println(dNum + 100); // 223.23
