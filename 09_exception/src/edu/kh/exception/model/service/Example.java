...
//		String input = br.readLine(); // == sc.nextLine();
		...
		// -> readLine() 메서드는 IOExeption을 던질 가능성이 있기 때문에
		//    호출 시 반드시 예외처리를 하시오.
		// --> Checked Excpetion
		...
		// 	  필수적으로 처리하지 않아도 된다. == Unchecked Exeption
	...
	public void ex2() {
		// Scanner 유사 객체(속도는 더 빠름)
				...
				try {
					...
					// catch : try 구문내에서 예외 객체가 던져진 경우
					//		   매개 변수(참조형)을 이용해서 해당 객체를 잡음.
	...
	public void ex4() {
		...
		try {
			...
			e.printStackTrace();
			// e : 예외 객체 참조 변수
			// printStackTrace : 예외가 발새한 메서드를 순서를 출력
		} 
